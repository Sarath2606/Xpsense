      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {group.members?.map((member) => {
            ...
            const mockBalance = Math.random() * 50 - 25; // Random balance between -25 and 25
            const isOwed = mockBalance > 0;
            const isOwing = mockBalance < 0;
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <div className="text-white text-sm font-medium">
                      {member.name} {member.id === 'current_user' || member.email === currentUser?.email ? '(you)' : ''}
                    </div>
                ...
                <div className="text-right">
                  {isOwed ? (
                    <div className="text-green-400 text-xs">
                      gets back {formatCurrency(mockBalance)}
                    </div>
                  ) : isOwing ? (
                    <div className="text-orange-400 text-xs">
                      owes {formatCurrency(Math.abs(mockBalance))}
                    </div>
                  ) : (
